    ...
    try:
        ...
        for r in recs:
            ...
            if sym == variety:
                ...
                break
        if not target:
            return {"status": "empty", "hint": "现货/基差（AKShare futures_spot_price）", "items": []}
        ...
        return {
            "status": "unavailable",
            "hint": f"现货/基差（接口异常：{type(e).__name__}）",
            "items": [],
            "params": {"date": date_compact},
        }
...
def _fetch_roll_yield(ak: Any, *, variety: str, date_compact: str) -> Dict[str, Any]:
    ...
    try:
        df = ak.get_roll_yield(date=date_compact, var=variety)
        recs = _to_records(df)
        items = recs[:]
        summary = None
        if items:
            # best-effort pick a numeric column
            r0 = items[0]
            val = None
        ...
        return {
            "status": "unavailable",
            "hint": f"展期收益率（接口异常：{type(e).__name__}）",
            "items": [],
            "params": {"date": date_compact, "var": variety},
        }
...
def _fetch_positions_rank(ak: Any, *, variety: str, date_compact: str) -> Dict[str, Any]:
    ...
    try:
        df = ak.get_rank_sum_daily(start_day=date_compact, end_day=date_compact, vars_list=[variety])
        recs = _to_records(df)
        items = recs[:]
        summary = None
        if items:
        ...
        return {
            "status": "unavailable",
            "hint": f"会员持仓/成交排名（接口异常：{type(e).__name__}）",
            "items": [],
            "params": {"date": date_compact, "var": variety},
        }
